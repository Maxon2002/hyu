...
let reviewsSliderWidth = reviewsSlider.offsetWidth
// let reviewBlockWidth = document.querySelector('.review-block').offsetWidth
const reviewsBlocks = reviewsSlider.querySelectorAll('.review-block')

let reviewIndex = 0

const nextReview = document.querySelector('.slider-button-next')
const prevReview = document.querySelector('.slider-button-prev')

function updateReviewsPosition() {
    const offset = -reviewIndex * reviewsSliderWidth;
...
prevReview.addEventListener('click', () => {
    reviewIndex = (reviewIndex - 1 + reviewsBlocks.length) % reviewsBlocks.length;
    updateReviewsPosition();
});

nextReview.addEventListener('click', () => {
    reviewIndex = (reviewIndex + 1) % reviewsBlocks.length;
    updateReviewsPosition();
});

function changeReviewsWidth() {
    reviewsSliderWidth = reviewsSlider.offsetWidth
    updateReviewsPosition()
}
...
// пермещение к блоку бронирования при нажатии на кнопки 

const bookButtons = document.querySelectorAll(".header-button-book");
const bookBlock = document.querySelector(".reservation-section");

bookButtons.forEach(button => {
    button.addEventListener("click", function () {

        bookBlock.scrollIntoView({ behavior: "smooth" });
